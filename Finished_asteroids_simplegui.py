...
def group_group_collide(set_group, other_set_group):
    collision_count = 0
    for item in set(set_group):
        if group_collide(other_set_group, item):
            collision_count += 1
            set_group.discard(item)
...
class Ship:
    ...
    def shoot(self):
        root = math.sqrt(self.forward[0] ** 2 + self.forward[1] ** 2)
        normalized_forward = [self.forward[0]/root,self.forward[1]/root]
        a_missile = Sprite([self.pos[0] + self.image_size[0] * 0.5 * normalized_forward[0],
                            self.pos[1] + self.image_size[1] * 0.5 * normalized_forward[1]],
...
def draw(canvas):
    ...
    canvas.draw_text("Score: " + str(score), [WIDTH - 110, 30], 30, 'red')
    ...
    if group_collide(rock_group, my_ship):
        lives -= 1
    collision_nr = group_group_collide(rock_group, missile_group)
    score += collision_nr
...
def rock_spawner():
    velocity = [random.randint(-30,30)/10.0, random.randint(-30,30)/10.0]
    position = [random.random()*WIDTH, random.random()*HEIGHT]
    ang_velocity = random.randint(-30,30) / 100.0
    a_rock = Sprite(position, velocity, 0, ang_velocity, asteroid_image, asteroid_info)
    if len(rock_group) <= 12 and dist(a_rock.get_position(), my_ship.get_position()) >= 100:
